...
# EasyOCR loads from this script, so we can show a loading message and avoid freezing the UI, it takes a sometimes over 2 minutes load depending on machine.
...
# Settings menu details can be found in instruictions menu
settings = {
    "show_preview": False,
    "sat_remove":   1,
    "max_sat":      50,
...
def save_to_csv(result, ocr_mode, roi_label):
    """Append one row for every OCR result while saving is active."""
...
# Polygon helpers to create a unique shape for the photo area
...
# Settings window extrernal window that u can tweak your settings if u want for better results.
...
while True:
    ...
    # DRAW MODE — render frozen frame + overlay (polygon photos to capture unique shapes)   
    if draw_mode:
